...
//            $p->SetFont('Helvetica', 'B', 6);
    ...
    public function BuatIsiTranskrip()
    {
        ...
        foreach ($krs_collection as $n => $krs) {
            if ($n >= $barisTotal) break;
            $p->setX(6);
            $p->Cell(4, $ti, $n+1, 'L', 0, 'C');
            $p->Cell(19, $ti, $krs['kode_matakuliah'], 'L', 0, 'C');
            $p->Cell(54, $ti, $krs['nama_matakuliah'], 'L', 0);
            $p->Cell(5.5, $ti, $krs['sks'], 'L', 0, 'C');
            $p->Cell(6.5, $ti, $krs['huruf_mutu'], 'L', 0, 'C');
            $p->Cell(6, $ti, number_format($krs['angka_mutu'], 2, ".", ","), 'L', 0, 'C');
            $p->Cell(5.5, $ti, $krs->getJumlahBobot(), 'LR', 0, 'C');
